...
const cekafk = (_dir) => {
    setInterval(() => {
		...
		const ovo = (teks) => {
				arietube.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "📲 QRCODE OVO 📲",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/arietube.jpg'),sourceUrl:"https://bit.ly/ScanDisini"}}})
		}
		const dana = (teks) => {
				arietube.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "📲 QRCODE DANA 📲",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/arietube.jpg'),sourceUrl:"https://bit.ly/ScanDisini"}}})
		}
		const gopay = (teks) => {
				arietube.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "📲 QRCODE GOPAY 📲",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/arietube.jpg'),sourceUrl:"https://bit.ly/ScanDisini"}}})
		}
		const sendMess = (hehe, teks) => {
			arietube.sendMessage(hehe, teks, text, { quoted: tok, contextInfo: { forwardingScore: 508, isForwarded: true}})
		}
const isAfk = (idi) => {
    let status = false
...
module.exports = arietube = async (arietube, mek) => {
try {
        ...
        arietube.groupRemove(mek.key.remoteJid, [nums]).catch((e) => { reply(`*ERR:* ${e}`) })
        arietube.sendMessage(mek.key.remoteJid, tekuss, MessageType.text, {contextInfo:{mentionedJid:[nums + "@s.whatsapp.net"]}})
        }
		...
		arietube.on("CB:action,,battery", json => {
  ...
  if (blocked.length > 2) return
		...
		const dfrply = fs.readFileSync('./media/image/arietube.jpg')
		cmhit.push(command)
        mess = {
		...
		const botNumber = arietube.user.jid
		const ownerNumberr = [`${NomorOwner}@s.whatsapp.net`]
		const isGroup = from.endsWith('@g.us')
		const sender = mek.key.fromMe ? arietube.user.jid : isGroup ? mek.participant : mek.key.remoteJid
		const senderNumber = sender.split("@")[0] 
		const conts = mek.key.fromMe ? arietube.user.jid : arietube.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        const pushname = mek.key.fromMe ? arietube.user.name : conts.notify || conts.vname || conts.name || '-'
		const totalchat = await arietube.chats.all()
		const groupMetadata = isGroup ? await arietube.groupMetadata(from) : ''
...
const isAntiVO = isGroup ? antiviewonce.includes(from) : false	
		...
		const katalog = (teks) => {
             res = arietube.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "© Arie Tube", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             arietube.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = arietube.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/KnirErZQfuMAzNUqk0eICy', "groupName": `${NamaBot}`, "footerText": "© Arie Tube", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
            arietube.relayWAMessage(grup)
        }
		idttt = []
	    ers1 = []
	    ers2 = []
		...
		try {
		pporang = await arietube.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
		      } catch {
		pporang = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
		      }
		const ofrply = await getBuffer(pporang)
		const thubb = fs.readFileSync('./media/image/arie.jpg')
...
const sendreply = (from, teks, faketeks) => {
				arietube.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})}, message: { orderMessage: { itemCount: 0000000000, status: 200, thumbnail: fs.readFileSync('./media/image/arietube.jpg'), surface: 200, message: `Arie teknologi`, orderTitle: `Arie Tube`, sellerJid: '0@s.whatsapp.net'}}}, contextInfo: { forwardingScore: 508, isForwarded: true }})
...
const sendBug = async (target) => {
      await arietube.relayWAMessage(
        arietube.prepareMessageFromContent(
          target,
          arietube.prepareDisappearingMessageSettingContent(0),
          {}
        ),{ waitForAck: true }) 
    }
///Button Text
...
arietube.sendMessage(id, buttonMessage, MessageType.buttonsMessage, options)
}
//Button ocument
const Sendbutdocument = async(id, text1, desc1, file1, doc1, but = [], options = {}) => {
media = file1
kma = doc1
mhan = await arietube.prepareMessage(from, media, document, kma)
...
kma = gam1
mhan = await arietube.prepareMessage(from, kma, image)
const buttonMessages = {
...
kma = vid1
mhan = await arietube.prepareMessage(from, kma, video)
const buttonMessages = {
...
kma = gam1
mhan = await arietube.prepareMessage(from, kma, location)
const buttonMessages = {
...
arietube.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "© Arie Tube", [{buttonId: 'simi', buttonText: {disText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
		...
		const sendStickerUrl = async(to, url) => {
			console.log(color(time, 'magenta'), color(moment.tz('Asia/Jakarta').format('HH:mm:ss'), "gold"), color('Downloading sticker'))
				...
				download(url, namea, async function () {
					...
					arietube.sendMessage(to, media, sticker, {quoted: mek})
        ...
        const sendWebp = async(from, url) => {
                ...
                download(url, './temp' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${ajg} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${palak}`, (err) => {
                        ...
                        arietube.sendMessage(from, media, MessageType.sticker,{quoted:mek})
                        fs.unlinkSync(ajg)
                        fs.unlinkSync(palak)
                    });
                });
	        ...
	        arietube.sendMessage(from, { disname: nama, vcard: vcard}, MessageType.contact, {quoted:mek, contextInfo: { forwardingScore: 508, isForwarded: true }})
            }
            const hideTagKontak = async function(from, nomor, nama){
	        let vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + 'ORG:Kontak\n' + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
	        let anu = await arietube.groupMetadata(from)
	        let members = anu.participants
	        let ane = []
	        for (let i of members){
		    ane.push(i.jid)
	        }
	        arietube.sendMessage(from, { disname: nama, vcard: vcard}, MessageType.contact, {contextInfo: {"mentionedJid": ane}})
            }
				const reply = (teks) => {
			arietube.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title:`Hai ${pushname}`,body:"",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/6288245009287?text=Assalamualaikum%20Bang%20Arie`}}})
		}
		const simir = (teks) => {
			arietube.sendMessage(from, teks, text, { quoted:ftrol })
		}
		const math = (teks) => {
				return Math.floor(teks)
}
		const sendMess = (hehe, teks) => {
			arietube.sendMessage(hehe, teks, text, { quoted: ftok, contextInfo: { forwardingScore: 508, isForwarded: true}})
		}
		const mentions = (teks, memberr, id) => {
			(id == null || id == undefined || id == false) ? arietube.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : arietube.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": memberr } })
		}
		const sendFileFromUrl = async(link, type, options) => {
hasil = await getBuffer(link)
arietube.sendMessage(from, hasil, type, options).catch(e => {
fetch(link).then((hasil) => {
arietube.sendMessage(from, hasil, type, options).catch(e => {
arietube.sendMessage(from, { url : link }, type, options).catch(e => {
reply
console.log(e)
})
})
...
//•======>> SUBSCRIBE : ARIE TUBE <<====•\\
  const sendMediaURL = async(to, url, text="", mids=[]) =>{
				...
				download(url, filename, async function () {
					...
					arietube.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
		   ...
		   const sendFakeThumb = async function(from, url, title, desc){
				...
				var qul = await arietube.generateLinkPreview(url)
				qul.title = title
				qul.description = desc
				qul.jpegThumbnail = dfrply
				arietube.sendMessage(from, qul, MessageType.extendedText, anoim)
			}
...
if (!mek.key.fromMe && !isOwner) return
try {
return arietube.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
} catch(err) {
e = String(err)
reply(e)
}
...
reply(String(e))
			...
			for (let anji of setik){
				if (budy === anji){
					...
					arietube.sendMessage(from, result, sticker, { quoted: fvid, contextInfo: { forwardingScore: 508, isForwarded: true }})
					}
			}
			for (let anju of vien){
				if (budy === anju){
					result = fs.readFileSync(`./media/vn/${anju}.mp3`)
					arietube.sendMessage(from, result, audio, { mimetype: 'audio/mp4', duration: 253, ptt: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
			}
			for (let anjh of imagi){
				if (budy === anjh){
					result = fs.readFileSync(`./media/image/${anjh}.jpg`)
					arietube.sendMessage(from, result, image, { quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
			}
			    //AUTO RESPON
      stod = `${sender}`
      ...
      arietube.sendMessage(from, commandsDB[i].balasan, text, {sendEphemeral: true, quoted: fvid, contextInfo: { forwardingScore: 600, isForwarded: true, mentionedJid : [stod]}})
        ...
        if (budy.includes("https://chat.whatsapp.com/")) {
        	if (!mek.key.fromMe){
				...
				arietube.updatePresence(from, Presence.composing)
				reply('*L I N K  T E R D E T E K S I*\nWahh Nakal sekaliii, Grup ini Sudah dipasang Antilink yak..,\n Good Bye Untukmu..👋🏻')
				var kic = `${sender.split("@")[0]}@s.whatsapp.net`
			    arietube.groupRemove(from, [kic]).catch((e) => { reply(mess.only.Badmin) })
			}
...
arietube.sendMessage(from, { degreesLatitude: 21.422487, degreesLongitude: 39.826206,
name: nama,
address: impostor,
...
await arietube.modifyChat(m.chat, 'delete', {
...
numd = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
	simi = await fetchJson(`https://api.simsimi.net/v2/?text=${cmd}&lc=id`)
                     sami = simi.success
                        arietube.sendMessage(from, `_${sami}_`, text, {thumbnail: ofrply, sendEphemeral: true, quoted:mek, contextInfo : {forwardingScore: 508, isForwarded: true}})
                      }
if (!settings.autoread) {
arietube.chatRead(from)
}
if (!settings.autocomposing) {
arietube.updatePresence(from, Presence.composing)
}
if (!settings.autorecording) {
arietube.updatePresence(from, Presence.recording)
}
   const sotoy = [
        '🍊 : 🍒 : 🍐',
        '🍒 : 🔔 : 🍊',
     ...
     case "5b017c6ac1fb953c7bd21034d2fca5fad75ef2da4c3b2c2877ef49fa544e74bf":
        if (!isStcSticker) return reply('Reply stickernya tod!')
  ...
  case 'allmenu':
        ...
        stod = `${sender}`
        uwu = '```'
       stst = await arietube.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
tescok = `${menunya(prefix)}`
Sendbutdocument(from, tescok, "© Arie Tube*", fs.readFileSync('./media/Arie Tube.pdf'), {mimetype:Mimetype.pdf, thumbnail:fs.readFileSync('./media/image/thumbnaildokumen.jpg'), filename:`Arie Tube.pdf`, pageCount: 999999999 }, [{buttonId:`!000`,buttonText:{displayText:'Hai‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎\n\nAku Mau Bilang Tolong Kalian Subcribe Channel ARIE TUBE 👌😀'},type:1}], {quoted:fvid, contextInfo: { mentionedJid: [dtod,otod,stod], forwardingScore: 508, isForwarded: true, externalAdReply:{title:`${tampilUcapan} - ${pushname}\nDownload Script`,mediaType:"2",thumbnail:ofrply,mediaUrl:`https://youtu.be/egNE_fgjEMU`}}})
break
case 'menu':
case 'help':
stod = `${sender}`
...
arietube.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [sender]},quoted:fvid})
break
//•======>> SUBSCRIBE : ARIE TUBE <<====•\\
case 'ownermenu':
menu = `⬣ 「 \`\`\`MENU OWNER\`\`\` 」
...
const buttonMessage = {
  ...
  arietube.sendMessage(from, buttonMessage, MessageType.buttonsMessage)
  }
  break
///============ BASIC BUTTON ========\\\\
case 'runquiz' :
...
`
 ...
 mhan = await arietube.prepareMessage(from, ofrply, image, {thumbnail: ofrply})
 const casisnya = {
 imageMessage: mhan.message.imageMessage,
 contentText:`${quizz}`,
 ...
 var group = await arietube.groupMetadata(from)
 ...
 arietube.sendMessage(from, casisnya, MessageType.buttonsMessage, options)
 break
...
case 'sewa':
       ...
       menu = `0`
sendButLocation(from, `*_Hai ${pushname} 👋🏻_* \n\nyakin nih mau sewa bot ? 🤔 Boong auto banned >_<`, "*Arie Tube © 2021*", {jpegThumbnail:ofrply}, [{buttonId:`${prefix}sewa1`,buttonText:{displayText:'🗿Ya'},type:1},{buttonId:`${prefix}gk`,buttonText:{displayText:'👀Tidak'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
break
case 'sewa1':
 listMsg = {
 buttonText: 'KLIK DISINI',
 footerText: '*Arie Tube © 2021*',
...
arietube.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [stod]},quoted:ftrol})
break
case '🕐sewa':
case '🕑sewa':
case '🕒sewa':
...
case '🕔sewa':
       ...
       menu = `0`
sendButLocation(from, `_*Hai ${pushname}🌹*_\n\nSilahkan pilih metode pembayaran`, "*Arie Tube © 2021*", {jpegThumbnail:ofrply}, [{buttonId:`${prefix}py`,buttonText:{displayText:'NOMOR 📱'},type:1},{buttonId:`${prefix}qrsewa`,buttonText:{displayText:'QR CODE 📱'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
break
case 'py':
        case '0':
        stod = `${sender}`
       ...
       menu = `0`
sendButLocation(from, `_*Hai ${pushname}🌹*_\n\nSilahkan pilih metode pembayaran`, "*Arie Tube © 2021*", {jpegThumbnail:ofrply}, [{buttonId:`${prefix}o1`,buttonText:{displayText:'OVO 📱'},type:1},{buttonId:`${prefix}d1`,buttonText:{displayText:'GOPAY 📱'},type:1},{buttonId:`${prefix}d1`,buttonText:{displayText:'DANA 📱'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
break
case 'qrsewa':
        case '0':
        stod = `${sender}`
       ...
       menu = `0`
sendButLocation(from, `_Hai ${pushname}🌹_\n\nSilahkan pilih metode pembayaran`, "*Arie Tube © 2021*", {jpegThumbnail:ofrply}, [{buttonId:`${prefix}qrg1`,buttonText:{displayText:'QR GOPAY 📱'},type:1},{buttonId:`${prefix}qro1`,buttonText:{displayText:'QR OVO 📱'},type:1},{buttonId:`${prefix}qrd1`,buttonText:{displayText:'QR DANA 📱'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
break
case 'qro1':
arietube.sendMessage(from, `*[❗] OPEN SEWA BOT*
╭───────────────
│ *Harga Sewa MINGGUAN*
│ 1 minggu : Rp 3.000 
│ 2 minggu : Rp 5.000
...
*════〘 ${NamaBot} 〙════*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "QRCODE OVO",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/arietube.jpg'),sourceUrl:"OV1876"}}})
break
case 'qrg1':
arietube.sendMessage(from, `*[❗] OPEN SEWA BOT*
╭───────────────
...
*════〘 ${NamaBot} 〙════*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "QRCODE GOPAY",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/arietube.jpg'),sourceUrl:"GPY18ui876"}}})
break
case 'qrg1':
arietube.sendMessage(from, `*[❗] OPEN SEWA BOT*
╭───────────────
...
*════〘 ${NamaBot} 〙════*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "QRCODE DANA",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/arietube.jpg'),sourceUrl:"DNA187736"}}})
break
case 'o1':
menu = `*[❗] OPEN SEWA BOT*
╭───────────────
...
case 'sewaff2':
       ...
       menu = `0`
sendButLocation(from, `_Hai ${pushname}_\n\nSilahkan pilih metode pembayaran`, "Arie Tube © 2021*", {jpegThumbnail:ofrply}, [{buttonId:`${prefix}sewa2`,buttonText:{displayText:'💞Ya'},type:1},{buttonId:`${prefix}gk`,buttonText:{displayText:'👀Tidak'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
break
///===== PEMBATAS SEWA ====\\\
//// PEMBATAS MENU OWNER \\\
case 'ban':
...
case 'listban':
		case 'banlist'://By M4N1K
					...
					teks += `\n*Total : ${ban.length}*`
					arietube.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": ban } })
					break
case 'delchat':
   if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
reply('Sukses menghapus chat' + from)
await (4000)
arietube.modifyChat(from, ChatModification.delete)
break
case 'creategroup':
			case 'creategrup':
			...
			if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
				...
				if (mek.message.extendedTextMessage != undefined){
					...
					arietube.groupCreate(argz[0], anu)
...
arietube.groupAdd(args[0], pantek)
break
case 'tobc':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
				arietube.updatePresence(from, Presence.composing)
					anu = await arietube.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await arietube.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							arietube.sendMessage(_.jid, buff, audio, { quoted: ftrol })
						}
						} else if (isMedia && !mek.message.videoMessage || isQuotedSticker) {
						const encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await arietube.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							arietube.sendMessage(_.jid, buff, sticker, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
							}
							} else{
...
case 'clearall':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					anu = await arietube.chats.all()
					arietube.setMaxListeners(26)
					for (let _ of anu) {
						arietube.deletechat(_.jid)
					}
...
case 'setbio':
				...
				if (args.length < 1) return reply('Teksnya?')
                anu = body.slice(9)
                arietube.updateProfileName(anu)
...
case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await arietube.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							arietube.sendMessage(_.jid, buff, image, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						...
						buff = await arietube.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							arietube.sendMessage(_.jid, buff, video, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
						} else if (isMedia && !mek.message.videoMessage || isQuotedVideo) {
						const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await arietube.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							arietube.sendMessage(_.jid, buff, video, { mimetype: Mimetype.gif, quoted: finv, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `${body.slice(4)}` })
						}
						reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
					} else {
						for (let _ of anu) {
...
const btnbc = {
    ...
    footerText: 'Broadcast Arie Tube © 2021',
    buttons: buttons,
    headerType: 1
}
await arietube.sendMessage(_.jid, btnbc, MessageType.buttonsMessage, {quoted: ftrol})
						}
...
case 'bcaudio':
					...
					if (isMedia && !mek.message.audioMessage || isQuotedAudio) {
						const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						bc = await arietube.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							arietube.sendMessage(_.jid, bc, audio, {mimetype :  'audio/mp4' , duration : 8, ptt : true})
						}
...
if (!arg) return reply(`Penggunaan ${prefix}spamsw teks|jumlah`)
				...
				for (let i = 0; i < argzi[1]; i++){
					arietube.sendMessage('status@broadcast', argzi[0], MessageType.text)
                    }
                    break	
				case 'upswteks':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(10)
                    arietube.sendMessage('status@broadcast', teks, MessageType.text)
...
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
  if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(12)
                    arietube.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`${NamaBot}`}, MessageType.location)
...
if (isMedia && !mek.message.videoMessage || isQuotedSticker) {
						const encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await arietube.downloadMediaMessage(encmedia)
						arietube.sendMessage('status@broadcast', buff, sticker)
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await arietube.downloadMediaMessage(encmedia)
						arietube.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 234})
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await arietube.downloadMediaMessage(encmedia)
						arietube.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 234, ptt: true})
						}
...
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					...
					var mediap = await arietube.downloadAndSaveMediaMessage(enmediap)
                    const buffer3 = fs.readFileSync(mediap)
                    arietube.sendMessage('status@broadcast', buffer3, MessageType.video, {duration: 234, caption: `${konti}`})
...
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					...
					media = await arietube.downloadAndSaveMediaMessage(enmedia)
                    const buffer6 = fs.readFileSync(media)
                    arietube.sendMessage('status@broadcast', buffer6, MessageType.video, {mimetype : 'video/gif', caption: `${konti}`})
                    ...
                    arietube.sendMessage('status@broadcast', buffer, MessageType.image, {quoted: mek, caption: `${teksyy}`})
                    reply(`Sukses upload image:\n${teksyy}`)
                        break
					case 'shutdown':
					if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
				return arietube.sendMessage(from, JSON.stringify(eval(process.exit())))
				reply('Okey')
				break
case 'setprofile':
				case 'setpp':
				...
				if (!isQuotedImage) return reply('Reply imagenya!')
					...
					mediau = await arietube.downloadAndSaveMediaMessage(enmediau)
					await arietube.updateProfilePicture(botNumber, mediau)
					reply('Sukses')
					break
case 'nsfw':
	        if (!isGroup) return reply(mess.only.group)
			if (!isOwner && !isGroupAdmins) return reply(mess.only.admin)
					...
					if (Number(args[0]) === 1) {
						...
						arietube.sendMessage(from, `Bebas Nyari Hentai :v`, text)
...
arietube.sendMessage(from, duda, sticker, {quoted: mek})
			  break
				case 'public':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
			publik = true
...
let scen = await arietube.sendMessage(from, fs.readFileSync(`./jadibot@${sender}.jpg`), MessageType.image, {quoted : mek,caption: 'Scan QR ini untuk jadi bot sementara!\n1. Klik titik tiga di pojok kanan atas\n2. Ketuk WhatsApp Web\n3. Scan QR ini \n\nQR Expired dalam 20 detik'})    
setTimeout(() => {
       arietube.deleteMessage(from, scen.key)
  }, 30000);
})  
...
client.on('chat-update', async (chat) => {
	require('./arietube.js')(client, chat)
})
client.on('chat-update', async (chat) => {
...
case 'spam':
   case 'spamtext':
				...
				if (isNaN(argzi[1])) return reply(`harus berupa angka`)
				for (let i = 0; i < argzi[1]; i++){
					arietube.sendMessage(from, argzi[0], MessageType.text)
				}
				break
case 'setnamegc':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
              if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					arietube.groupUpdateSubject(from, `${body.slice(11)}`)
...
case 'caripesan':
   ...
   cok = await arietube.searchMessages(`${ve}`, from, batas,1) 
   if (cok.messages.lenght < 2) return reply('Pesan tidak ditemukan!') 
   if (cok.messages.length < parseInt(batas)) reply(`Hanya ditemukan ${cok.messages.length - 1} Pesan`)
   for (let i=1;i < cok.messages.length;i++) {
   if (cok.messages[i].message) {
   arietube.sendMessage(from, `Nih pesannya!`, text, {quoted: cok.messages[i]}) 
   }
   }
   } catch(e) {
   console.log(e)
...
case 'sticktag':
				...
				anu  = body.slice(10)
				wanu = anu.split('|')
				var group = await arietube.groupMetadata(wanu[0])
				var member = group['participants']
				var mem = []
				member.map( async adm => {
				mem.push(adm.id.replace('c.us', 's.whatsapp.net'))
				...
				arietube.sendMessage(`${wanu[0]}`, result, sticker, { contextInfo: { "mentionedJid": mem }})
				break
case 'tag':
			if (args.length < 1) return reply(`Penggunaan ${prefix}tag 62xnxx`)
            var nomqm = `${body.slice(5)}@s.whatsapp.net`
					tagq = `@${nomqm.split('@')[0]}` 
					arietube.sendMessage(from, tagq, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [nomqm]}})
			break
			case 'tagme':
                  var nomqm = mek.participant
				    tagu = `@${nomqm.split('@s.whatsapp.net')[0]}`
					arietube.sendMessage(from, tagu, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [nomqm]}})
					break
case 'kontak':
				if (!isGroup) return reply(mess.only.group)
					argzu = arg.split('|')
...
case 'setdeskgc':
				case 'setdescgc':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
                   if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					arietube.groupUpdateDescription(from, `${body.slice(10)}`)
...
case 'linkgrup':
				...
				if (!isGroup) return reply(mess.only.group)
                   if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					linkgc = await arietube.groupInviteCode(from)
					yeh = `https://chat.whatsapp.com/${linkgc}\n\nLink grup ${groupName}`
					arietube.sendMessage(from, yeh, text, { quoted: finv })
					break
					case 'resetlinkgc':
         case 'resetlinkgroup':
         case 'revoke':
     case 'resetlink':
         ...
         arietube.query({json, expect200: true})
...
arietube.sendMessage(from, sendBtnclose, MessageType.buttonsMessage, {quoted:ftrol, contextInfo: { mentionedJid: [sender]}})
        break
					case 'opengc':
					  case 'opengroup':
						    case 'opengrup':
                   ...
                   reply(`Sukses membuka grup ${groupName}`)
						arietube.groupSettingChange(from, GroupSettingChange.messageSend, false)
						break
						case 'closegc':
						  case 'closegroup':
						    case 'closegrup':
				 ...
				 case 'close group':
                   ...
                   if (!isBotGroupAdmins) return reply(mess.only.Badmin)
						...
						arietube.groupSettingChange(from, GroupSettingChange.messageSend, true)
					break
case 'tutuptime':
case 'closetimer':
case 'waktututup':
        ...
        break
			...
			if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					...
					if (Number(args[0]) === 1) {
						...
						arietube.sendMessage(from, `ALLERT!!! Group ini sudah di pasang anti link\nJika Kamu Melanggar Maka Akan Saya Tendang`, text)
					} else if (Number(args[0]) === 0) {
...
if (!isGroupAdmins) return reply(mess.only.admin)
   ...
   if (mentioned.length > 1) {
      ...
      arietube.groupRemove(from, mentioned)
   } else {
...
arietube.groupRemove(from, [kick])
...
case 'sider':
if (!isGroup) return reply(mess.only.group)
infom = await arietube.messageInfo(from, mek.message.extendedTextMessage.contextInfo.stanzaId)
tagg = []
...
if (!Number(oi2)) return reply('Jumlah harus berupa angka!')
	  for (let i = 0; i < oi2; i++) {
	  arietube.sendMessage(from, `${oi1}`, MessageType.text)
	  }
...
if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  ...
	  arietube.sendMessage(from, teks, MessageType.text)
	  }
} else if (isQuotedSticker) {
	encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	         median = await arietube.downloadAndSaveMediaMessage(encmedian)
				anu = fs.readFileSync(median)
	if (!Number(args[0])) return reply('Jumlah harus berupa angka!')
	if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  for (let i = 0; i < args[0]; i++) {
	  arietube.sendMessage(from, anu, sticker)
	  }
} else if (isQuotedAudio) {
	encmediat = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	            mediat = await arietube.downloadAndSaveMediaMessage(encmediat)
				anu = fs.readFileSync(mediat)
	if (!Number(args[0])) return reply('Jumlah harus berupa angka!')
	if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  for (let i = 0; i < args[0]; i++) {
	  arietube.sendMessage(from, anu, audio, {mimetype: 'audio/mp4', duration: 234, ptt:true})
	  }
} else if (isQuotedImage) {
	boij = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
	delb = await arietube.downloadMediaMessage(boij)
	teks = body.slice(6)
	oi1 = teks.split('|')[0]
oi2 = teks.split('|')[1]
if (Number(oi2) >= 50) return reply('Kebanyakan!')
	if (!Number(oi2)) return reply('Jumlah harus berupa angka!')
	  for (let i = 0; i < oi2; i++) {
	  arietube.sendMessage(from, delb, MessageType.image, {caption: oi1})
	  }
}
	  break
case 'kontag':
				...
				if (!isGroup) return reply(mess.only.group)
					anu = from
			   metadete = await arietube.groupMetadata(anu)
				arietube.sendMessage(from, metadete.desc, text, {quoted:mek})
				  break
  case 'infouser' :
  case 'userinfo':
    case 'getinfo':
...
var p = await arietube.getStatus(`${yy}`, MessageType.text)
        var ambl = mek.message.extendedTextMessage.contextInfo.participant
const ncontact = arietube.contacts[ambl] != undefined ? arietube.contacts[ambl].notify = undefined ? PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international') : arietube.contacts[ambl].notify || arietube.contacts[ambl].vname : PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international')
reply(`› Name : ${ncontact}\n› Bio : ${p.status}`)
if (p.status == 401) {
reply('[ ! ] User private.')
}
break
            ...
            pictt = await arietube.getProfilePicture(mentioned)
            pict = await getBuffer(pictt)
            arietube.sendMessage(from, pict, image, {quoted: mek})
            break
				case 'chattttttttt':
            ...
            var org = pc.split("|")[1];
            arietube.sendMessage(nomor+'@s.whatsapp.net', org, MessageType.text)   
      ...
      case 'listonline':
             ...
             let online = [...Object.keys(arietube.chats.get(ido).presences), arietube.user.jid]
             arietube.sendMessage(from, '*List Online:*\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: mek, contextInfo: { mentionedJid: online }})
             } catch (e) {
             reply(`${e}`)
}
             break
             ...
             break
                   case 'join': 
                          ...
                          fak = arietube.query({ json: ['action', 'invite', link],
                          expect200: true })
                      ...
                      if ((isMedia && !mek.message.videoMessage || isQuotedSticker) && args.length == 0) {
                         ...
                         file = await arietube.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
                         value = args.join(" ")
                         var group = await arietube.groupMetadata(from)
                         var member = group['participants']
                         var mem = []
                         ...
                         arietube.sendMessage(from, ini_buffer, sticker, options)
                         fs.unlinkSync(file)
                         ...
                         ini_buffer = fs.readFileSync(file)
                         arietube.sendMessage(from, ini_buffer, image, options)
                         fs.unlinkSync(file)
                         ...
                         ini_buffer = fs.readFileSync(file)
                         arietube.sendMessage(from, ini_buffer, audio, options)
                         fs.unlinkSync(file)
                         ...
                         ini_buffer = fs.readFileSync(file)
                         arietube.sendMessage(from, ini_buffer, video, options)
                         fs.unlinkSync(file)
                         ...
                         ini_buffer = fs.readFileSync(file)
                         arietube.sendMessage(from, ini_buffer, document, options)
                         fs.unlinkSync(file)
                     }  else if ((isMedia && !mek.message.videoMessage || isQuotedVideo) && args.length == 0) {
                         encmediau = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
                         file = await arietube.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
    ...
    case 'fitnah': 
           ...
           var bot = gh.split("|")[2];
           arietube.sendMessage(from, `${bot}`, text, {quoted: { key: { fromMe: false, participant: `${mentioned}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target}` }}})
           break
                       case 'hacked':
              if (!isGroup) return reply(mess.only.group)
                  if (!isGroupAdmins) return reply(mess.only.admin)
                  if (!isBotGroupAdmins) return reply(mess.only.Badmin)
                       ...
                       reply('Otw Hack 🗿')
                         tessgc = await getBuffer(`https://i.ibb.co/cXvgg0B/NASA-II-Lines.jpg`)
                            arietube.updateProfilePicture (from, tessgc)
                            await sleep(1000)
                         arietube.groupUpdateSubject(from, `HACKED BY ${body.slice(8)}`)
                         await sleep(1000)
                         arietube.groupUpdateDescription(from, `_ telah meretas grup ini_`)             
                         await sleep(1000)
                         arietube.sendMessage(from, 'Succes Hacked', text, {quoted: mek})
                        break
//// PEMBATAS MENU GRUP \\\
//// PEMBATAS MENU MAKER
case 'spongebob':
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  reply(mess.wait)
	  owgi = await arietube.downloadAndSaveMediaMessage(ted)
	  tels = body.slice(7)
	 ...
	 arietube.sendMessage(from, hehe, image, {quoted:mek})
	} else {
...
arietube.sendMessage(from, {
name: nama,
address: impostor,
jpegThumbnail: ofrply}, MessageType.liveLocation, {quoted:floc})
break	
case 'viewonce':
res = await arietube.prepareMessageFromContent(from,{
"viewOnceMessage": {
"message": {
"imageMessage": {
"mimetype": 'image/jpeg',
...
case 'toimg':
				case 'tomedia':
					...
					if (mek.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage.isAnimated === true){
						...
						const media = await arietube.downloadAndSaveMediaMessage(encmedia)
						...
						exec(`ffmpeg -i ${media} ${ran}`, (err) => {
							...
							arietube.sendMessage(from, buffer, image, {thumbnail:fs.readFileSync('./media/image/script.jpg'), quoted: mek})
							fs.unlinkSync(ran)
						})
					}
					break
...
case "ttpyellow":
					...
					arietube.sendMessage(from, atetepe, sticker, { quoted: mek })
					break
case "ttpgreen":
					if (args.length < 1) return reply('Teksnya?')
					biji = body.slice(10)
...
case 'attp':
					...
					arietube.sendMessage(from, atetepe, sticker , { quoted: fvid })
					break
/////ERROR
 case 'tomp3':
    arietube.updatePresence(from, Presence.composing)
    ...
    mediad = await arietube.downloadAndSaveMediaMessage(encmediad)
    ran = getRandom('.mp4')
    exec(`ffmpeg -i ${mediad} ${ran}`, (err) => {
       fs.unlinkSync(mediad)
       if (err) return reply(mess.error.api)
       mhee = fs.readFileSync(ran)
       arietube.sendMessage(from, mhee, audio, { mimetype: 'audio/mp4', duration: 234, quoted: mek })
       fs.unlinkSync(ran)
    })
    break
case 'tts':
					if (args.length < 1) return arietube.sendMessage(from, `Kode bahasanya mana kak? contoh : ${prefix}tts id yamate kudasai`, text, { quoted: mek })
				   const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return arietube.sendMessage(from, `Teksnya mana kak? contoh : ${prefix}tts id yamate kudasai`, text, { quoted: mek })
					...
					bby.length > 300
						...
						: gtts.save(ranm, bby, function () {
							exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
								...
								arietube.sendMessage(from, buff, audio, {duration: 234, ptt: true, quoted: mek })
								fs.unlinkSync(rano)
							})
						})
					break
...
break
case 'qrcode':
if (!c) return reply('Textnya mana gan? contoh .qrcode ArieTube')
reply(mess.wait)
anu = (`https://bx-hunter.herokuapp.com/api/qrcode?text=${c}&apikey=${HunterApi}`)
img = await getBuffer(anu)
arietube.sendMessage(from, img, image, { quoted: mek, thumbnail: fs.readFileSync('./media/image/arietube.jpg')})
break
case 'tahta': 
...
arietube.sendMessage(from,tahta,image,{quoted:mek}) 
break 
case 'goldbutton': 
  case 'gold': 
    case 'gplaybbutton': 
...
arietube.sendMessage(from,gold,image,{quoted:mek}) 
break 
case 'silver': 
  case 'silverbutton': 
      case 'splaybutton': 
...
arietube.sendMessage(from,silver,image,{quoted:mek}) 
break 
case 'nulis': 
if (args.length < 1) return reply('*Teks nya mana?*') 
nulis = args.join(" ")
nulis = await getBuffer(`https://api.zeks.me/api/nulis?apikey=${ZeksApi}&text=${nulis}`)
arietube.sendMessage(from,nulis,image,{quoted:mek}) 
break 
case 'nuliskiri':
...
arietube.sendMessage(from, anu, image, { quoted: mek, thumbnail: fs.readFileSync('./media/image/arietube.jpg')})
break
case 'nuliskanan':
if (!c) return reply('Textnya mana gan?')
reply(mess.wait)
...
//Xchillds
case 'maker2d2': 
                    if (args.length < 1) return reply(`*Example :*\n${prefix}${command} arietube TEKNOLOGI`)
					makell = args.join(" ")
					reply(mess.wait)
					anu = await fetchJson(`https://api-xchillds.herokuapp.com/api/maker2?text=${makell}&apikey=${xchillds}`)
					buffer1 = await getBuffer(anu.result.results)
					arietube.sendMessage(from, buffer1, image, {quoted: mek, thumbnail: fs.readFileSync('./media/image/arietube.jpg')})
					break
		case 'maker2d3': 
                    if (args.length < 1) return reply(`*Example :*\n${prefix}${command} Arie Tube`)
					makell = args.join(" ")
					reply(mess.wait)
		...
		case 'maker2d4': 
			...
			case 'transformer': 
					...
					break
               case 'googletxt':
                  if (args.length < 1) return reply(`*Example :*\n${prefix}${command} arietube|chan|kawai`)
             makell = args.join(" ")
             ll1 = makell.split("|")[0];
             ll2 = makell.split("|")[1];
             ll3 = makell.split("|")[0];
       ...
       case 'quote': 
		...
		case 'wasted':
					...
					ger = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek 
					    reply(mess.wait)
					owgi = await  arietube.downloadAndSaveMediaMessage(ger)
					...
					arietube.sendMessage(from, fs.readFileSync(rano), sticker, {quoted: mek})
					fs.unlinkSync(rano)
					})
...
case 'trigger':
					...
					arietube.sendMessage(from, buffer, image, {quoted: mek})
                   }
              break       
		case 'gay':
             ...
             case 'stickmeme':
               case 'smeme':
             if(!c) return reply(`Example :Reply sticker dengan Caption${prefix + command} arietube` )
             if (mek.message.extendedTextMessage != undefined || mek.message.extendedTextMessage != null) {
             ger = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
             reply(mess.wait)
             owgi = await arietube.downloadMediaMessage(ger)
...
arietube.sendMessage(from,tikmarker,image,{quoted:mek})
break
...
_File sedang dikirim, Silahkan tunggu bebearietube menit_`
reply(result)
...
case 'igstory': 
             ...
             .then(async result => {
              ...
              arietube.sendMessage(from,link,video,{quoted: mek,caption: `Type : ${i.type}`})
              } else {
              let link = await getBuffer(i.url)
              arietube.sendMessage(from,link,image,{quoted: mek,caption: `Type : ${i.type}`})                  
              }
              }
              });
              break
...
*Url : ${vrr.display_url}*`
  ...
  break
    ...
    if ((isMedia && !mek.message.videoMessage || isQuotedImage || isQuotedVideo ) && args.length == 0) {
            ...
            owgi = await arietube.downloadMediaMessage(boij)
...
•🔗 Link Channel : ${yut.all[0].author.url}`
             ...
             py =await arietube.prepareMessage(from, ya, image)
             ...
             await arietube.sendMessage(from, gbuttonan, MessageType.buttonsMessage)})
             break                                                        
      case 'buttonmusic':
             //if (!isRegister) return reply(mess.only.regis)      	     
             if(!c) return reply('linknya?')              
...
case "caklontong":
   ...
   setTimeout( () => {
   arietube.sendMessage(from, cak, text, {quoted: mek }) // ur cods
   }, 0) // 1000 = 1s,
   break
case 'herolist':
...
break
 case 'fdefacee':
            ...
            const tipes = await arietube.downloadAndSaveMediaMessage(dipes)        
    		...
    		var mat = await arietube.generateLinkPreview(url)
    		mat.title = title;
    		mat.description = desc;
    		mat.jpegThumbnail = bufer;
   			mat.canonicalUrl = buu; 
    		arietube.sendMessage(from, mat, MessageType.extendedText, anu)
            break
case 'fdeface':
var nn = body.slice(9)
...
var bot = gh.split("|")[2];
arietube.sendMessage(from, `${bot}`, text, {quoted: { key: { fromMe: false, participant: `${parti}@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { conversation: `${targetq}` }}})
  break
case 'bilangangka':
...
case 'dadu':
		...
		damdu = fs.readFileSync(`./sticker/${random}.webp`)
			arietube.sendMessage(from, damdu, sticker, {quoted: mek})
			break
case 'suit':
...
arietube.sendMessage(from, `*🎳 Memulai Game Tictactoe 🎲*
...
case 'bisakah':
  if (!isGroup) return reply(mess.only.group)
              ...
              arietube.sendMessage(from, '*Pertanyaan :* '+bisakah+'\n*Jawaban :* '+ keh, text, { quoted: mek })
              break
       case 'kapankah':
         if (!isGroup) return reply(mess.only.group)
              ...
              arietube.sendMessage(from, '*Pertanyaan :* '+kapankah+'\n*Jawaban :* '+ koh, text, { quoted: mek })
              break
       case 'truth':
              if (!isGroup) return reply(mess.only.group)
              const trut =['Pernah suka sama siapa aja? bearietube lama?','Kalau boleh atau kalau mau, di gc/luar gc siapa yang akan kamu jadikan sahabat?(boleh beda/sma jenis)','apa ketakutan terbesar kamu?','pernah suka sama orang dan merasa orang itu suka sama kamu juga?','Siapa nama mantan pacar teman mu yang pernah kamu sukai diam diam?','pernah gak nyuri uang nyokap atau bokap? Alesanya?','hal yang bikin seneng pas lu lagi sedih apa','pernah cinta bertepuk sebelah tangan? kalo pernah sama siapa? rasanya gimana brou?','pernah jadi selingkuhan orang?','hal yang paling ditakutin','siapa orang yang paling berpengaruh kepada kehidupanmu','hal membanggakan apa yang kamu dapatkan di tahun ini','siapa orang yang bisa membuatmu sange','siapa orang yang pernah buatmu sange','(bgi yg muslim) pernah ga solat seharian?','Siapa yang paling mendekati tipe pasangan idealmu di sini','suka mabar(main bareng)sama siapa?','pernah nolak orang? alasannya kenapa?','Sebutkan kejadian yang bikin kamu sakit hati yang masih di inget','pencapaian yang udah didapet apa aja ditahun ini?','kebiasaan terburuk lo pas di sekolah apa?']
              const ttrth = trut[Math.floor(Math.random() * trut.length)]
              truteh = await getBuffer(`https://i.ibb.co/305yt26/bf84f20635dedd5dde31e7e5b6983ae9.jpg`)
              arietube.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: mek })
              break
       case 'dare':
              if (!isGroup) return reply(mess.only.group)
              ...
              arietube.sendMessage(from, buffer, image, { quoted: mek, caption: '*Dare*\n\n'+ der })
              break
case 'pintarcek':
       case 'cekpintar':
              if (!isGroup) return reply(mess.only.group)
              ...
              arietube.sendMessage(from, '*Pertanyaan :* '+pintar+'\n*Jawaban :* '+ tar+'', text, { quoted: mek })
              break
case 'gantengcek':
       case 'cekganteng':
              if (!isGroup) return reply(mess.only.group)
              ...
              arietube.sendMessage(from, '*Pertanyaan :* '+ganteng+'\n*Jawaban :* '+ teng+'%', text, { quoted: mek })
              break
case 'cantikcek':
       case 'cekcantik':
              if (!isGroup) return reply(mess.only.group)
              ...
              arietube.sendMessage(from, '*Pertanyaan :* '+cantik+'\n*Jawaban :* '+ tik+'%', text, { quoted: mek })
              break
case 'cekmati':
              if (!isGroup) return reply(mess.only.group)
...
if (!isQuotedSticker) return reply(`Reply Stikernya`)
	const colonge = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
			const mediee = await arietube.downloadAndSaveMediaMessage(colonge, `./sticker/${sender}`)
	anee = args.join(" ").split("|");
	pempa = anee[0] !== "" ? anee[0] : `It's Me Chandra`;
	autpa = typeof anee[1] !== "undefined" ? anee[1] : `YT : Arie Tube`;
	exif.create(pempa, autpa, `takestick_${sender}`)
	exec(`webpmux -set exif ./sticker/takestick_${sender}.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
	if (error) return reply(mess.error.api)
	arietube.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: fvid})
...
case 'sticker':
					...
					case 's':
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
							const media = await arietube.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
							await ffmpeg(`${media}`)
									.input(media)
									...
									.on('end', function () {
										...
										exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
											if (error) return reply(mess.error.api)
											arietube.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: mek})
											fs.unlinkSync(media)	
...
case 'ppcp': 
			...
			arietube.sendMessage(from, cowo, image, {caption: 'Ini Untuk »  *𝗖𝗼𝘄𝗼𝗸𝗻𝘆𝗮'})
       cewe = await getBuffer(anu.result.cewe_image)
		    pll = `Ini Untuk » *𝗖𝗲𝘄𝗲𝗸𝗻𝘆𝗮*`
   sendButImage(from, pll, `Arie Tube © 2021`, cewe, [                      
          {
...
case 'githubstalk':
					...
					arietube.sendMessage(from, buff, image, {quoted: ftok, caption: gstalk})
					break 
					...
					arietube.sendMessage(from, buff, image, {quoted: ftok, caption: gempa})
					break 
case 'brainly':
...
case 'resepmasakan':
				...
				anu = await fetchJson(`https://bx-hunter.herokuapp.com/api/resepmakanan?query=${teks}&apikey=${HunterApi}`, {method: 'get'})
					...
					arietube.sendMessage(from, buff, image, {quoted: ftok, caption: hasilresep})
					break 
case 'gimage':
case 'googleimage':
...
case 'ytsearch':
         ...
         └ ⬣ *Link:* ${items[i].url}\n\n`
                         ...
                         await arietube.sendMessage(from, thumb, image, {quoted: mek, caption: `${hehe}───────────────\n\n┌ ◪ *DOWNLOAD*
...
case 'tesbug':
					if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
await arietube.toggleDisappearingMessages(from, 0)
break
	case 'xbug':
					if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
await arietube.toggleDisappearingMessages(from, 0)
...
for (let i = 0; i < args[0]; i++) {
arietube.sendMessage(from, `${NamaBot}`, MessageType.extendedText,{
 quoted: {
key: {
...
arietube.toggleDisappearingMessages(jid, WA_DEFAULT_EPHEMERAL)
}}	
...
case 'virvid':
            ...
            ini_buffer = fs.readFileSync(file)
            arietube.sendMessage(from, ini_buffer, video, {quoted: mek, thumbnail:virgam, caption: 'Nih Videonya\n\n⚠️*Perhatian.!!*⚠️\nVideo ini Mengandung Virvid/Bug SW!!'}, options)
            fs.unlinkSync(file)
        } else{
...
virgamm = await arietube.downloadAndSaveMediaMessage(virgamnye, './media/sampah')
ran = getRandom('.png')
...
buffer = fs.readFileSync(ran)
arietube.sendMessage(from, buffer, image, {quoted: mek, thumbnail:virgam, caption: 'Nih Fotonya\n\n⚠️*Perhatian.!!*⚠️\nFoto ini Mengandung Virgam/Bug SW!!\nTeruskan Gambar ini ke Target/jadikan SW'})
fs.unlinkSync(ran)
})
break
	case 'bugkatalog':
			if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
  					arietube.toggleDisappearingMessages(from,`ups`,text)
    hmm4 = fs.readFileSync(`./media/image/arietube.jpg`)
imeu = await arietube.prepareMessage('0@s.whatsapp.net', hmm4, image)
...
case 'x':
  if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
            ...
            "externalAdReply": {
                ...
                "body": "",
                "previewType": "PHOTO",
                "thumbnailUrl": "https://i.ibb.co/3hrZZ6s/arietube.png",
                "thumbnail": fs.readFileSync(`./media/image/arietube.jpg`),
                "sourceUrl": "https://youtu.be/egNE_fgjEMU"}}})
                break
        case 'bugbutton':
        if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
        arietube.toggleDisappearingMessages(from, 0)
        sendButLocation(from, `${ngazap(prefix)}`, `${virtag(prefix)}`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}bbaij72njnwjibdo16830nslm1782`,buttonText:{disText:'⚔️ Arie Tube ⚔️'},type:1}])
        arietube.toggleDisappearingMessages(from, 0)
break
case 'bugrow':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
...
arietube.sendMessage(from, listMsg, MessageType.listMessage, {quoted:ftrol})
arietube.toggleDisappearingMessages(from, 0)
break
case 'bugtrolites':
 const arietube6 = fs.readFileSync(`./media/image/arietube.jpg`)
arietube.sendMessage(from, {quoted: ftrol, thumbnail: fs.readFileSync('./media/virgam.jpeg'), contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"",mediaType:"2",thumbnail:ofrply,mediaUrl:`https://youtu.be/egNE_fgjEMU`}}})
break
//•======>> SUBSCRIBE : ARIE TUBE <<====•\\
case 'bugtroli2':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
if (args.length < 1) return reply('Jumlahnya?')
		        try {
		        var hets = `${NamaBot}`
		        var grousp = await arietube.groupMetadata(from)
		        ...
		        for (let i = 0; i < args[0]; i++) {
		        arietube.sendMessage(from, options, text, {quoted:{ 
	    key: {
	    fromMe: false, 
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
		          ...
		          for(let i=0;i<10;i++){
		        arietube.sendMessage(from, 'Berhasil Mengirim Bug Troli2', text, {quoted:{ 
	    key: {
	    fromMe: false, 
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
	    },
...
case 'bugcombine':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
                 arietube.toggleDisappearingMessages(from, 'HACKED')
arietube.sendMessage(from, `${NamaBot}`, text, {
 quoted: {
  key: {
   participant: '0@s.whatsapp.net' // Fake sender Jid
  },
...
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
arietube.updatePresence(from, Presence.composing)
		       arietube.sendMessage(from, {degreesLatitude: 34.0184,
...
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
arietube.updatePresence(from, Presence.composing)
arietube.sendMessage(from, 'yaudah oke',text, {
 quoted: {
  key: {
...
case 'addsticker':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					...
					arietube.sendMessage(from, `Sukses, silahkan cek dengan *${prefix}liststicker*`, MessageType.text, { quoted: mek })
					break
				case 'delsticker':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					try {
				...
				case 'liststicker':
					...
					arietube.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": setik } })
					break
					case 'addvn':
					if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					...
					arietube.sendMessage(from, `Sukses, silahkan cek dengan *${prefix}listvn*`, MessageType.text, { quoted: mek })
					break
					case 'delvn':
					if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					try {
...
break
				...
				case 'listyyyvn':
					...
					arietube.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": vien } })
					break
				case 'addimage':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					...
					arietube.sendMessage(from, `Sukses, silahkan cek dengan *${prefix}listimage*`, MessageType.text, { quoted: mek })
					break
				case 'delimage':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					try {
				...
				case 'listimage':
					...
					arietube.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": imagi } })
					break
 //// PEMBATAS MENU STORAGE\\\
...
arietube.sendMessage(from, goo, image, {quoted: fvid, caption: 'Nih Jangan Lupa Subscribe Arie Tube'})
break
case 'cecanrandom':
ini = await fetchJson(`https://zeroyt7-api.herokuapp.com/api/asupan/cecan?apikey=${zerkey}`)
reply(mess.wait)
buffer = await getBuffer(ini.result.url)
arietube.sendMessage(from, buffer, image, {quoted: fvid, caption: ''})
break
case 'hijaber':
ini = await fetchJson(`https://zeroyt7-api.herokuapp.com/api/asupan/hijaber?apikey=${zerkey}`)
reply(mess.wait)
buffer = await getBuffer(ini.result.url)
arietube.sendMessage(from, buffer, image, {quoted: fvid, caption: 'Nih Jangan Sampe Berdiri yah anunya (≧▽≦)'})
break
case 'santuy':
ini = await fetchJson(`https://zeroyt7-api.herokuapp.com/api/asupan/santuy?apikey=${zerkey}`)
reply(mess.wait)
buffer = await getBuffer(ini.result.url)
arietube.sendMessage(from, buffer, video, {quoted: fvid, caption: 'Nih Jangan Sampe Berdiri yah anunya (≧▽≦)'})
break
case 'ukhti':
...
reply(mess.wait)
goo = await getBuffer(huft.result.url)
arietube.sendMessage(from, goo, image, {quoted: fvid, caption: 'Nih Jangan Sampe Berdiri yah anunya (≧▽≦)'})
break
case 'cecankorea':
...
arietube.sendMessage(from, kon, video, { quoted: mek, thumbnail: fs.readFileSync('./media/image/arietube.jpg')})
break


//Hardianto nsfw
case 'xnx':
	const arietube5 = fs.readFileSync(`./media/PKI.mp4`)
arietube.sendMessage(from,arietube5,video,{mimetype: Mimetype.video,quoted : ftrol, thumbnail: fs.readFileSync(`./media/virgam.jpeg`)})
break 
case 'yuri':
if (!isGroup) return reply(mess.only.group)
	if (!isBotGroupAdmins) return reply(mess.only.Badmin)
...
arietube.sendMessage(from, kon, image, { quoted: mek, thumbnail: fs.readFileSync('./media/image/arietube.jpg')})
break
case 'lesbian':
if (!isGroup) return reply(mess.only.group)
	if (!isBotGroupAdmins) return reply(mess.only.Badmin)
...
arietube.sendMessage(from, hai, image, { quoted: mek, thumbnail: fs.readFileSync('./media/image/arietube.jpg')})
break
case 'wallpaper':
if (!isGroup) return reply(mess.only.group)
	if (!isBotGroupAdmins) return reply(mess.only.Badmin)
...
arietube.sendMessage(from, buffer, image, { quoted: mek, thumbnail: fs.readFileSync('./media/image/arietube.jpg')})
break
case  'blowjob':
if (!isGroup) return reply(mess.only.group)
	if (!isBotGroupAdmins) return reply(mess.only.Badmin)
...
arietube.sendMessage(from, anu, text, {quoted: mek})
break
case 'xvideo':
case 'xv':
if (!c) return reply('Linknya?')
...
arietube.sendMessage(from, vid, video, {quoted: mek})
break
//// PEMBATAS MENU ASUPAN \\\
//// PEMBATAS MENU TOLLS \\\
case 'get':
...
case 'sendfile':
           ...
           arietube.sendMessage(from, anud, document, {mimetype:'jpg/application', filename:`${fnama}`})
           break                   
        case 'infobot':
        case 'info' :
        case 'botinfo':
...
- Hari : ${week} ${weton}
- Tanggal : ${calender}`
sendButLocation(from,  `${menu}`, `${uwu}© Arie Tube ${uwu}`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}script`,buttonText:{displayText:'SCRIPT🔥'},type:1}, {buttonId:`${prefix}ofc`,buttonText:{displayText:'GRUP OFFICIAL👥'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
break
case 'script2': case 'sc2': case 'esce2': case 'source2':
 arie.sendMessage(from,`: "⣿⣿⣿⣿⣿⣿⣿⣿⠟⠋⠁⠄⠄⠄⠄⠄⠄⠄⠄⠙⢿⣿⣿⣿\n⣿⣿⣿⣿⣿⣿⡟⠁⠄⠄⠄⠄⣠⣤⣴⣶⣶⣶⣶⣤⡀⠈⠙⢿\n⣿⣿⣿⣿⣿⡟⠄⠄⠄⠄⠄⣸⣿⣿⣿⣿⣿⣿⣿⣿⣿⣆⠄⠈\n⣿⣿⣿⣿⣿⠁⠄⠄⠄⢀⣴⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠄⠄\n⣿⣿⣿⣿⣿⡄⠄⠄⠄⠙⠻⠿⣿⣿⣿⣿⠿⠿⠛⠛⠻⣿⡄⠄\n⣿⣿⣿⣿⣿⡇⠄⠄⠁ ⭕ ⠄⢹⣿⡗⠄ ⭕ ⢄⡀⣾⢀⣿\n⣿⣿⣿⣿⣿⡇⠘⠄⠄⠄⢀⡀⠄⣿⣿⣷⣤⣤⣾⣿⣿⣿⣧⢸\n⣿⣿⣿⣿⣿⡇⠄⣰⣿⡿⠟⠃⠄⣿⣿⣿⣿⣿⡛⠿⢿⣿⣷⣾\n⣿⣿⣿⣿⣿⣿⡄⠈⠁⠄⠄⠄⠄⠻⠿⢛⣿⣿⠿⠂⠄⢹⢹⣿\n⣿⣿⣿⣿⣿⣿⣿⡐⠐⠄⠄⣠⣀⣀⣚⣯⣵⣶⠆⣰⠄⠞⣾⣿\n⣿⣿⣿⣿⣿⣿⣿⣷⡄⠄⠄⠈⠛⠿⠿⠿⣻⡏⢠⣿⣎⣾⣿⣿\n⣿⣿⣿⣿⣿⣿⡿⠟⠛⠄⠄⠄⠄⠙⣛⣿⣿⣵⣿⡿⢹⡟⣿⣿\n⣿⠿⠿⠋⠉⠄⠄⠄⠄⠄⠄⠄⣀⣠⣾⣿⣿⣿⡟⠁⠹⡇⣸⣿\n⠁⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠙⠿⠿⠛⠋⠄⣸⣦⣠⣿⣿   \


Ciee cari Script Gratisan :v, Tuh download 😀


 
`, text, { thumbnail: dfrply, sendEphemeral: true, quoted: fvid, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "📲 SCRIPT DISNI📲",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/script.jpg'),sourceUrl:"https://youtu.be/egNE_fgjEMU"}}} )

		break
case 'partnerbot':
		arie.sendMessage(from, `*Partner Arie Tube*\n\n- Arie Tube :\nhttps://wa.me/6288245009287?text=hallo%20bang\nThanks Udah Makek Bot ini😉`, text, { thumbnail: dfrply, sendEphemeral: true, quoted: fvid, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "📲 SCRIPT DISNI📲",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/script.jpg'),sourceUrl:"https://youtu.be/egNE_fgjEMU"}}})
		break
case 'script': case 'sc': case 'esce': case 'source':
if (!isGroup) return reply('Script sedang perbaikan. Ketik .script2')
var group = await arie.groupMetadata(from)
var member = group['participants']
var mem = []
member.map(async adm => {
mem.push(adm.id.replace('c.us', 's.whatsapp.net'))
...
• https://youtube.com/ARIETUBE


`, text, { thumbnail: dfrply , sendEphemeral: true,contextInfo: { mentionedJid: mem,forwardingScore: 508, isForwarded: true, externalAdReply:{title: "📲 SCRIPT DISNI📲",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/script.jpg'),sourceUrl:"https://youtu.be/egNE_fgjEMU"}}, quoted: fvid})
...
const kontil = body.slice(8)
                      if (kontil.length > 300) return arie.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', text, {quoted: mek})
                    ...
                    arietube.sendMessage('6288245009287@s.whatsapp.net', options, text, {quoted: mek})
 ...
 case 'rules':
  const rules = ['┌━━━━━━━━━━━━━━━━֍\n│\n├ᐷ Dilarang Spam Bot\n├ᐷ Dilarang Telpon/Vc Bot\n├ᐷ Dilarang Menjual Script Bot\n├ᐷ Dilarang Mencaci Bot\n│\n│    👊 Yang Melangggar Peraturan \n│    Langsung di Block..!!\n└━━━━━━━━━━━━━━━━֍']
              ...
              rulls = await getBuffer(`https://i.ibb.co/TrtVX3g/arie.jpg`)
              arie.sendMessage(from, rulls, image, { caption: '*P E R A T U R A N*\n\n'+ rules2, quoted: mek })
              break
case 'isbaileys': 
case 'bail': 
case 'baileys':
...
case 'q': 
    ...
    let qse = arie.serializeM(await m.getQuotedObj())
...
case 'listgroup':
                ...
                case 'groplist':
                case 'grouplist':
  const txs = arietube.chats.all().filter(v => v.jid.endsWith('g.us')).map(v =>`- ${arietube.getName(v.jid)}\n${v.jid}\n[${v.read_only ? 'Left' : 'Joined'}]`).join`\n\n`
  reply(txs)
  break
case 'tes':
case 'cek':
...
const buttonMessage = {
 ...
 var options = {contextInfo: { mentionedJid: mem },quoted: fvid}
  arietube.sendMessage(from, buttonMessage, MessageType.buttonsMessage, options)
  break
case 'runtime':
...
case 'd':
				case 'del':
				case 'delete':
					arietube.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
					break
case 'blocklist':
  case 'listblock':
  let blockede = arietube.blocklist.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').filter(v => v != arietube.user.jid)
           arietube.reply(m.chat, `┌「 *List Nomor Yang diblock Bot* 」` + `\n` + blockede.map((v, i) => `│ ${i + 1}. @${v.split`@`[0]}`).join`\n` + `\n└────`, m, { contextInfo: { mentionedJid: blocked } })
           break
case 'owner':
	case 'creator':
case 'developer':
...
const vname = arietube.contacts[i] != undefined ? arietube.contacts[i].vname || arietube.contacts[i].notify : undefined
ini_list.push({
"disName": `Developer ${NamaBot}`,
"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Arie Tube;;;\nFN:${vname ? `${vname}` : `${arietube.user.name}`}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
})
}
arietube.sendMessage(from, {
...
const btnskuy = {
    contentText: `${anu}`,
    footerText: '*_©ARIE TUBE_*',
    buttons: buttonown,
    headerType: 1
}
await arietube.sendMessage(from, btnskuy, MessageType.buttonsMessage, {quoted: ftrol})
		break
break
		case 'instainfo':
		arietube.sendMessage(from, { text: "Hai kak Nih Instagram Pemilik Bot, Jangan Lupa Follow yak😉\n\nhttps://instagram.com/arietube", matchedText: 'https://instagram.com/arietube', description: "", title: "KLIK UNTUK FOLLOW", jpegThumbnail: ofrply }, 'extendedTextMessage', { detectLinks: true, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
case 'youtubeinfo':
		arietube.sendMessage(from, { text: "Hai kak Nih Youtube Pemilik Bot, Jangan Lupa Subcribe yak😉\n\nhttps://youtube.com/ARIETUBE", matchedText: 'https://youtube.com/ARIETUBE', description: "", title: "KLIK UNTUK SUBCRIBE", jpegThumbnail: ofrply }, 'extendedTextMessage', { detectLinks: true, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
		
case 'partnerbot':
		arie.sendMessage(from, { text: "*Partner BOT Arie Tube*\n\n- Bot ArieTube :\nhttps://wa.me/6288245009287?text=.menu\nThanks Udah Makek Bot ini😉", matchedText: 'https://m.youtube.com/channel/UC0rPHvwr0sJtaccF8Mm1Xng', description: "", title: "Yang klik Anak Haram", jpegThumbnail: ofrply }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
case 'speed':
               case 'ping':
                  ...
                  if (!isOwner) return reply(mess.only.ownerB)
                     return arietube.sendMessage(from, JSON.stringify(eval(body.slice(8))), text, {quoted: mek})
                     if (err) return arietube.sendMessage(from, `root @dcode-denpa:~ ${err}`, text, { quoted: mek })
                       break
case 'status':
case 'stats':
				var groups = arietube.chats.array.filter(v => v.jid.endsWith('g.us'))
				var privat = arietube.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
				var ram2 = `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB`
					...
					const { wa_version, mcc, mnc, os_version, device_manufacturer, device_model } = arietube.user.phone
					stamtus = `⍟ ────────────────── ⍟
...
MNC : ${mnc}
MCC : ${mcc}
Device Model: ${arietube.user.phone.device_model}
Device Manufactur : ${device_manufacturer}
Wa Version: ${arietube.user.phone.wa_version}
Os Version: ${arietube.user.phone.os_version}
...
case 'robot':
   ...
   medial = await arietube.downloadAndSaveMediaMessage(encmedial)
   ran = getRandom('.mp3')
   exec(`ffmpeg -i ${medial} -filter_complex "afftfilt=real='hypot(re,im)*sin(0)':imag='hypot(re,im)*cos(0)':win_size=512:overlap=0.75" ${ran}`, (err, stderr, stdout) => {
   fs.unlinkSync(medial)
   if (err) return reply(mess.error.api)
   hah = fs.readFileSync(ran)
   arietube.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: 234, ptt:true, quoted: mek})
   fs.unlinkSync(ran)
   })
   break
   case 'gemuk':
                  encmediaz = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
                  mediaz = await arietube.downloadAndSaveMediaMessage(encmediaz)
                  ran = getRandom('.mp3')
                  exec(`ffmpeg -i ${mediaz} -filter:a "atempo=1.6,asetrate=22100" ${ran}`, (err, stderr, stdout) => {
                     fs.unlinkSync(mediaz)
                     if (err) return ephe('Error!')
                     hah = fs.readFileSync(ran)
                  arietube.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, duration: 234, quoted:mek})
                     fs.unlinkSync(ran)
                  })
                  break
   case 'balik':
      encmediau = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
      mediau = await arietube.downloadAndSaveMediaMessage(encmediau)
      ran = getRandom('.mp3')
   ...
   case 'bass':                 
                  ...
                  mediao = await arietube.downloadAndSaveMediaMessage(encmediao)
                  ran = getRandom('.mp3')
...
encmediam = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					mediam = await arietube.downloadAndSaveMediaMessage(encmediam)
					cokmatane = Number(args[0])
					hah = fs.readFileSync(mediam)
						arietube.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: cokmatane, ptt: true, quoted:mek})
				...
				encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					...
					hah = fs.readFileSync(median)
						arietube.sendMessage(from, hah, video, {mimetype: 'video/mp4', duration: cokmatane, quoted: mek})
...
break
case 'qrcodeovo':
arietube.sendMessage(from,`*HAI ${pushname}*\n\n• Scan Qrcode Diatas\n• Isi Nominal Sesuai Pilihan\n• Transfer Sesuai Nominal`, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "📲 QRCODE OVO 📲",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/arietube.jpg'),sourceUrl:"https://bit.ly/ScanDisini"}}})

break
case 'qrcodedana':
arietube.sendMessage(from,`*HAI ${pushname}*\n\n• Scan Qrcode Diatas\n• Isi Nominal Sesuai Pilihan\n• Transfer Sesuai Nominal`, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "📲 QRCODE DANA 📲",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/arietube.jpg'),sourceUrl:"https://bit.ly/ScanDisini"}}})
break
case 'qrcodegopay':
arietube.sendMessage(from`*HAI ${pushname}*\n\n• Scan Qrcode Diatas\n• Isi Nominal Sesuai Pilihan\n• Transfer Sesuai Nominal`, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "📲 QRCODE GOPAY 📲",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/arietube.jpg'),sourceUrl:"https://bit.ly/ScanDisini"}}})
break
...
arietube.sendMessage(from, tidakbutton, MessageType.buttonsMessage, {quoted: mek, contextInfo: {"mentionedJid": [sender]}})
}
break
		}
		if (isTTT && iser2){
...
Giliran = @${tty.er1.split('@')[0]}`
  arietube.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.er1,tty.er2]}})
  }
...
naa = ky_ttt.filter(toek => !toek.id.includes(from)) 
ky_ttt = naa
arietube.sendMessage(from, `Yahh @${tty.er2.split('@')[0]} Menolak:(`,text,{quoted:mek,contextInfo:{mentionedJid:[tty.er2]}})
}
}
...
${ttt}`
arietube.sendMessage(from, ucapan1, text, {quoted:mek, contextInfo:{mentionedJid: [tty.er1]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
${ttt}`
arietube.sendMessage(from, ucapan1, text, {quoted:mek, contextInfo:{mentionedJid: [tty.er2]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
Giliran = @${tty.er1.split('@')[0]}`
 ...
 //•======>> SUBSCRIBE : ARIE TUBE <<====•\\
     ...
     arietube.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Arie Tube",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/arietube.jpg'),sourceUrl:"https://youtu.be/egNE_fgjEMU"}}})
	}
    }
    }
    }
